...
# dataset_dir = './Dataset'
flags.DEFINE_string('dataset_dir', './Dataset/', 'String: dataset directory')

#State where your log file is at. If it doesn't exist, create it.
# log_dir = '/home/lou/Feature_Embedding/2_Workspace/py3_virtualenv/training/Logs'
flags.DEFINE_string('log_dir', './ResNet_Logs', 'String: directory in which the logs should be written')

#State where your checkpoint file is
# checkpoint_file = './inception_resnet_v2_2016_08_30.ckpt'
flags.DEFINE_string('checkpoint_file', './checkpoints/inception_resnet_v2_2016_08_30.ckpt', 'String: path of the checkpoint')
...
flags.DEFINE_string('labels_file', './Dataset/labels.txt', 'String: path of the labels file')
FLAGS = flags.FLAGS
...
def get_split(split_name, dataset_dir, file_pattern=file_pattern):
    ...
    file_pattern_path = os.path.join(FLAGS.dataset_dir, file_pattern % (split_name))

    #Count the total number of examples in all of these shard
    num_samples = 0
    file_pattern_for_counting = 'feature_embedding_' + split_name
    tfrecords_to_count = [os.path.join(FLAGS.dataset_dir, file) for file in os.listdir(FLAGS.dataset_dir) if file.startswith(file_pattern_for_counting)]
    ...
    keys_to_features = {
      ...
      'image/num_channels': tf.FixedLenFeature(
          [], tf.int64, default_value=tf.zeros([], dtype=tf.int64)),
    }
    ...
    'num_channels': slim.tfexample_decoder.Tensor('image/num_channels'),
    }
    ...
    with tf.Graph().as_default() as graph:
		...
		# Save checkpoint for ever fifth epoch
                if step%int(num_steps_per_epoch) == 0:
                    intermediate_saver.save(sess, './ResNet_intermediate_Logs/model.ckpt', global_step = step)
